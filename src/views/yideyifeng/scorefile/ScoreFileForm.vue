...
/** 文件记录 表单 */
defineOptions({ name: 'ScoreFileForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await ScoreFileApi.getScoreFile(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as ScoreFile
    if (formType.value === 'create') {
      await ScoreFileApi.createScoreFile(data)
      message.success(t('common.createSuccess'))
    } else {
      await ScoreFileApi.updateScoreFile(data)
